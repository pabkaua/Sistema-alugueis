...
public class ItemBusiness implements IItemBusiness {
    ...
    public void cadastrar(Item i){
        if (repo.buscar(i.getId()) != null){
            throw new BusinessException("Item já cadastrado: " + i.getNome());
        }
        else if (catRepo.buscar(i.getCategoria().getId()) == null) {
            throw new BusinessException("Categoria não cadastrada: " + i.getCategoria().getNome());
        }
        else if (fornRepo.buscar(i.getFornecedor().getId()) == null){
            throw new BusinessException("Fornecedor não cadastrado: " + i.getFornecedor().getNome());
        }
    ...
    public Map<String, Item> listarPorCategoria(Categoria categoria){
        if (categoria == null){
            ...
            throw new BusinessException("Nome da categoria inválida!");
        } else if (catRepo.buscar(categoria.getId()) == null){
            throw new BusinessException("Categoria não cadastrada: " + categoria.getNome());
        }
    ...
    public Map<String, Item> listarPorFornecedor(Fornecedor fornecedor){
        if (fornecedor == null){
            ...
            throw new BusinessException("Fornecedor não cadastrado: " + fornecedor.getNome());
        }
    ...
    public void deletar(String id){
        ...
        if (repo.buscar(id).hasHistorico()){
            throw new BusinessException("O item não pode ser excluido, tem histórico");
        }
        repo.deletar(id);
    }
